 ...
 *
 * \details These error valus are returned from various uvgRTP functions.
 * Functions that return a pointer set rtp_errno global value that should be
 * checked if a function call failed
 */
typedef enum RTP_ERROR {
  ...
  RTP_SEND_ERROR =
      -5,  ///< System call send(2) or one of its derivatives failed
  ...
  RTP_RECV_ERROR =
      -11,            ///< System call recv(2) or one of its derivatives failed
  RTP_TIMEOUT = -12,  ///< Operation timed out
  RTP_NOT_FOUND = -13,  ///< Object not found
  RTP_AUTH_TAG_MISMATCH =
      -14,  ///< Authentication tag does not match the RTP packet contents
} rtp_error_t;
...
typedef enum RTP_FLAGS {
  ...
  /** Obsolete flags*/
  RTP_OBSOLETE = 1,
  RTP_SLICE = 1,  // used to do what RTP_NO_H26X_SCL does, may do something
                  // different in the future
  /// \endcond

  /** Make a copy of the frame and perform operation on the copy. Cannot be used
     with unique_ptr. */
  RTP_COPY = 1 << 1,

  /** By default, uvgRTP searches for start code prefixes (0x000001 or
   * 0x00000001) from the frame to divide NAL units and remove the prefix. If
   * you instead want to provide the NAL units without the start code prefix
   * yourself, you may use this flag to disable Start Code Lookup (SCL) and the
   * frames will be treated as send-ready NAL units. */
  RTP_NO_H26X_SCL = 1 << 2,
 ...
 *
 * \details These flags are passed to uvgrtp::session::create_stream and can be
 * OR'ed together
 */
enum RTP_CTX_ENABLE_FLAGS {
  ...
  // These can be used to specify what the address does for one address create
  // session
  RCE_SEND_ONLY = 1 << 1,  ///<  address/port interpreted as remote, no binding
                           ///<  to local socket
  RCE_RECEIVE_ONLY =
      1 << 2,  ///<  address/port interpreted as local, sending not possible
  ...
  /** By default, uvgRTP restores the stream by prepending 3 or 4 byte start
   * code to each received H26x frame, so there is no difference with sender
   * input. You can remove start code prefix with this flag */
  RCE_NO_H26X_PREPEND_SC = 1 << 6,

  /** Use this flag to discard inter frames that don't have their previous
     dependencies arrived. Does not work if the dependencies are not in
     monotonic order. */
  RCE_H26X_DEPENDENCY_ENFORCEMENT = 1 << 7,

  /** Fragment frames into RTP packets of MTU size (1492 bytes).
   *
   * Some RTP profiles define fragmentation by setting the marker bit indicating
   * the last fragment of the frame. You can enable this functionality using
   * this flag at both sender and receiver.
   */
  RCE_FRAGMENT_GENERIC = 1 << 8,

  /** Enable System Call Clustering (SCC). Sender side flag.

  The benefit of SCC is reduced CPU usage at the sender, but its cost is
  increased chance of losing frames at the receiving end due to too many packets
  arriving at once.*/
  ...
  /** Paces the sending of frame fragments within frame interval (default 1/30
     s) */
  RCE_PACE_FRAGMENT_SENDING = 1 << 20,

  /** Use a single UDP port for both RTP and RTCP transmission (default RTCP
     port is +1) **/
  RCE_RTCP_MUX = 1 << 21,
...
enum RTP_CTX_CONFIGURATION_FLAGS {
  ...
  /** How many milliseconds is each frame waited for until it is considered
   * lost.
   *
   * Default is 500 milliseconds
   *
   * This is valid only for fragmented frames,
   * i.e. RTP_FORMAT_H26X and RTP_FORMAT_GENERIC with RCE_FRAGMENT_GENERIC
   * (TODO) */
  RCC_PKT_MAX_DELAY = 4,
  ...
  * uvgRTP tries to send frames at this exact frame rate,
  ...
  2) if RCE_PACE_FRAGMENT_SENDING has been set, the fragments are set at a
  constant pace
  * spaced out evenly within frame interval */
   ...
   *
   * uvgRTP chooses this automatically depending on the format of the data being
   * transferred. Use this flag to manually set the session bandwidth in kbps.
   * RTCP reporting interval depends on this session bandwidth. The interval is
   ...
   *
   * Default value is 100 ms. If you are experiencing packet loss when
   * receiving, you can try lowering this value down to 0. This will, however
   * cause increased CPU usage in the receiver, so use with caution.
   */
  RCC_POLL_TIMEOUT = 13,
   ...
   * Default is 8.
   *
   * Used to compute the pace rate used when RCE_PACE_FRAGMENT_SENDING is
   * enabled
   */
