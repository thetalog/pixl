...
class _FollowOrUnfollow extends State<FollowOrUnfollow> {
  ...
  Future<void> checkFollowStatus() async {
    try {
      final url = Uri.parse(Config.buildApiUrl(
          ...
          onPressed: () async {
            ...
            try {
              if (isRequested && !isFollow) {
                ...
                final url = Uri.parse(Config.buildApiUrl(
                    '/follow/remove-follow-request?targetUsername=${widget.targetUsername}'));
                ...
                final response = await http.post(url,
                    ...
                    body: {"targetUsername": widget.targetUsername});
                final responseDecoded = jsonDecode(response.body);
                ...
                // Unfollow
                final url = Uri.parse(Config.buildApiUrl(
                    '/follow/remove-following?targetUsername=${widget.targetUsername}'));
                final response = await http
                    .patch(url, headers: {"Authorization": "Bearer $token"});
                final responseDecoded = jsonDecode(response.body);
