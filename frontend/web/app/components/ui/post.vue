<template>
  <article v-if="!dismissed" class="overflow-hidden rounded-card bg-pixl-card ring-1 ring-white/6">
    <div
      v-if="suggested"
      class="flex items-center justify-between border-b border-white/6 px-3 py-2"
    >
      <span class="text-xs font-semibold text-pixl-muted">
        {{ displayTopic }}
      </span>
      <button
        type="button"
        class="inline-flex h-7 w-7 items-center justify-center rounded-full text-pixl-muted hover:bg-white/6 hover:text-pixl-text"
    ...
    <div class="flex items-center gap-2 px-3 py-3">
      ...
      <div class="flex min-w-0 flex-1 items-center gap-2">
        <NuxtLink
          ...
          class="truncate text-sm font-semibold hover:text-pixl-accent-2"
        >
          {{ authorUserName }}
        </NuxtLink>
        <span v-else class="truncate text-sm font-semibold">Unknown</span>
        <UiFollowButton
          v-if="showFollowButton && !isOwn && authorUserName"
...
const displayTopic = computed(() => {
  const t = (props.topic || props.post?.topic || '').toString().trim()
  return t || 'Suggested for you'
})

function onFollowChange({ isFollow, isRequested }) {
  if (isFollow || isRequested) dismissed.value = true
}
