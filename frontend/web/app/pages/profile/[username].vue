...
const { data: liveResponse } = await useAsyncData(
  () => `profile-live:${username.value}`,
  () => liveApi.getByUsername(username.value),
  { server: false, watch: [username] }
)
const liveNow = computed(() => liveResponse.value?.stream || null)
...
function openAvatar() {
  if (liveNow.value?.id) {
    navigateTo(`/live/${encodeURIComponent(liveNow.value.id)}${isOwn.value ? '?host=1' : ''}`)
    return
  }
  picOpen.value = true
}
  ...
  <div class="mx-auto w-full max-w-[720px] px-4 py-6">
    ...
    <div v-else>
      <div class="flex flex-col gap-6 sm:flex-row sm:items-start">
        <button type="button" class="self-start" :aria-label="liveNow ? 'Watch live' : isOwn ? 'View profile photo' : 'Profile photo'" @click="openAvatar">
          <UiAvatar :src="profile.profilePic" :alt="profile.userName" :size="88" :live="!!liveNow" />
        </button>
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            <template v-if="isOwn">
              <UiButton v-if="liveNow?.id" size="sm" @click="navigateTo(`/live/${encodeURIComponent(liveNow.id)}?host=1`)">Resume live</UiButton>
