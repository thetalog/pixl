...
const send = async () => {
  ...
  sending.value = true
  try {
    // Backend expects multipart/form-data with field: postData
    const form = new FormData()
    form.append(
      'postData',
      JSON.stringify({ receiverUsername: targetUsername.value, message: text })
    )
...
<template>
  <div style="padding: 16px">
    <div style="display: flex; gap: 12px; align-items: center">
      ...
      <h1 style="margin: 0">Direct: {{ targetUsername }}</h1>
    </div>
