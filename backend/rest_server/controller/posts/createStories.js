const { dbCreateStories } = require("../../database/query/posts/createStories");
const { uploadPostOrReelToMinIO } = require("../object_storage/uploadFilesToMinIO");
const { getUserPostCount } = require("../../database/query/posts/getUserPostCount");
...
const createStoryController = async (req, res) => {
  try {
    ...
    const response = await dbCreateStories(
      user.id,
      taggedUsers,
      uploadResults
    );
