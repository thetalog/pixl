const prisma = require("../../lib/prisma");
const { ProfileVisibility } = require("@prisma/client");

function tokenizeQuery(q) {
  return String(q || "")
    .toLowerCase()
    .split(/[^a-z0-9]+/i)
    .map((t) => t.trim())
    .filter((t) => t.length >= 2);
}

function scorePost(post, tokens) {
  if (!tokens.length) return 0;
  const hay = [
    post.caption || "",
    ...(post.userTags || []),
    ...(post.systemTags || []),
    ...((post.media || []).flatMap((m) => m.labels || [])),
  ]
    .join(" ")
    .toLowerCase();

  let score = 0;
  for (const token of tokens) {
    if (hay.includes(token)) score += 3;
    // soft prefix boost inside labels
    for (const label of (post.media || []).flatMap((m) => m.labels || [])) {
      if (label === token) score += 4;
      else if (label.startsWith(token) || token.startsWith(label)) score += 1;
    }
  }
  return score;
}

function jaccard(a, b) {
  const A = new Set((a || []).map((x) => String(x).toLowerCase()));
  const B = new Set((b || []).map((x) => String(x).toLowerCase()));
  if (!A.size || !B.size) return 0;
  let inter = 0;
  for (const x of A) if (B.has(x)) inter += 1;
  return inter / (A.size + B.size - inter);
}

async function findPostsByNaturalLanguage(query, { take = 24 } = {}) {
  const tokens = tokenizeQuery(query);
  if (!tokens.length) {
    return { status: 200, data: [], message: "Empty query" };
  }

  const orFilters = tokens.flatMap((token) => [
    { caption: { contains: token, mode: "insensitive" } },
    { userTags: { has: token } },
    { systemTags: { has: token } },
    { media: { some: { labels: { has: token } } } },
  ]);

  const candidates = await prisma.post.findMany({
    where: {
      postDisabled: false,
      user: { profileVisibility: ProfileVisibility.PUBLIC },
      OR: orFilters,
    },
    include: {
      media: true,
      user: {
        select: {
          id: true,
          userName: true,
          name: true,
          profilePic: true,
        },
      },
    },
    orderBy: { createdAt: "desc" },
    take: Math.min(take * 4, 120),
  });

  const ranked = candidates
    .map((post) => ({ post, score: scorePost(post, tokens) }))
    .filter((x) => x.score > 0)
    .sort((a, b) => b.score - a.score || b.post.createdAt - a.post.createdAt)
    .slice(0, take)
    .map(({ post, score }) => ({ ...post, _searchScore: score }));

  return {
    status: 200,
    message: "AI image search results",
    data: ranked,
    query: tokens,
  };
}

async function findSimilarPostsByLabels(labels, { excludePostId, take = 24 } = {}) {
  const clean = [...new Set((labels || []).map((l) => String(l).toLowerCase().trim()).filter(Boolean))];
  if (!clean.length) {
    return { status: 200, data: [], message: "No labels to compare" };
  }

  const candidates = await prisma.post.findMany({
    where: {
      postDisabled: false,
      ...(excludePostId ? { id: { not: excludePostId } } : {}),
      user: { profileVisibility: ProfileVisibility.PUBLIC },
      OR: [
        { media: { some: { labels: { hasSome: clean } } } },
        { systemTags: { hasSome: clean } },
      ],
    },
    include: {
      media: true,
      user: {
        select: {
          id: true,
          userName: true,
          name: true,
          profilePic: true,
        },
      },
    },
    orderBy: { createdAt: "desc" },
    take: Math.min(take * 5, 150),
  });

  const ranked = candidates
    .map((post) => {
      const postLabels = [
        ...(post.systemTags || []),
        ...((post.media || []).flatMap((m) => m.labels || [])),
      ];
      return { post, score: jaccard(clean, postLabels) };
    })
    .filter((x) => x.score > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, take)
    .map(({ post, score }) => ({ ...post, _similarity: Number(score.toFixed(4)) }));

  return {
    status: 200,
    message: "Similar images",
    data: ranked,
    labels: clean,
  };
}

async function getPostLabels(postId) {
  const post = await prisma.post.findUnique({
    where: { id: postId },
    include: { media: true },
  });
  if (!post) return null;
  const labels = [
    ...(post.systemTags || []),
    ...((post.media || []).flatMap((m) => m.labels || [])),
  ];
  return { post, labels: [...new Set(labels.map((l) => String(l).toLowerCase()))] };
}

module.exports = {
  tokenizeQuery,
  findPostsByNaturalLanguage,
  findSimilarPostsByLabels,
  getPostLabels,
  jaccard,
};
